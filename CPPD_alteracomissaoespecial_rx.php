...
if(!empty($_POST)) {
	...
	$dataDistribuicao = $row['dataMovimentacao'];
...
?>
	...
	<div style="background-color: #1A5321;">
		...
		<div class="container" style="background-color: #E4EBE2;">
			<BR>
			<form action="CPPD_alteracomissaoespecial_rx2.php?idCPPD=<?php echo fnEncodeID($idCPPD);?>" method="post">
				<div class="row">
					<div class="coluna" align="center">
						<label>Docente requerente</label><br>
				...
				<div class="row">
					...
					<div class="coluna" align="center">
						...
						<input type="date" name="dataCPPD" readonly style="background-color: #E9E9E9;" value="<?php echo $dataDistribuicao;?>"/><br>
					</div>
				</div>
				<div class="row">
					<div class="coluna" align="center">
						...
						if ($numlinhas > 0) {
							while ($row = mysqli_fetch_assoc($result)) {
								...
								if (strcmp($ife,$ifedocente) != 0) {
									if (strcmp($nome,$presidente) == 0) {
?>
										<option selected><?php echo $nome;?></option>
<?php
									} else {
?>
...
<?php
						...
						if ($numlinhas > 0) {
							while ($row = mysqli_fetch_assoc($result)) {
								...
								$ife = $row['ife'];
								if (strcmp($ife,$ifedocente) != 0) {
									if (strcmp($nome,$membroexterno) == 0) {
?>
										<option selected><?php echo $nome;?></option>
<?php
									} else {
...
<?php
						...
						if ($numlinhas > 0) {
							while ($row = mysqli_fetch_assoc($result)) {
								...
								$ife = $row['ife'];
								if (strcmp($ife,$ifedocente) == 0) {
									if (strcmp($nome,$membrointerno) == 0) {
?>
										<option selected><?php echo $nome;?></option>
<?php
									} else {
