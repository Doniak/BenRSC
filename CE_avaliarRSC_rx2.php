...
if(!empty($_POST)) {
	...
	$siape = $_POST['siape'];
	$unidadesdeferida = $_POST['unidadesdeferida'];
	$observacao = $_POST['observacao'];
	$fatorpontuacao = $_POST['fatorpontuacao'];
	$pontdeferida = $unidadesdeferida * $fatorpontuacao;
	...
	$query = "SELECT criterio,qtdeunidades FROM $nomeTab WHERE id=$idCriterio";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_assoc($result);	
	$nrcriterio = $row['criterio'];
	$unidRequerida = $row['qtdeunidades'];
	
	if ($unidadesdeferida > $unidRequerida) {
		$unidadesdeferida = $unidRequerida;
		$pontdeferida = $unidadesdeferida * $fatorpontuacao;
	}
	
	$query = "SELECT qtdemaxunid FROM TabRSC WHERE criterio=$nrcriterio";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_assoc($result);	
	$unidMaximaCriterio = $row['qtdemaxunid'];

	if ($unidadesdeferida > $unidMaximaCriterio) {
		$unidadesdeferida = $unidMaximaCriterio;
		$pontdeferida = $unidadesdeferida * $fatorpontuacao;
	}	
	
	if (strcmp($presidente,$nomeAvaliador) == 0) 
	{
		$query = "UPDATE $nomeTab SET unidDefPresidente=$unidadesdeferida,PontDefPresidente=$pontdeferida,observPresidente='$observacao' WHERE id=$idCriterio";
		$result = mysqli_query($link, $query);
		$query = "UPDATE TabSolicitaRSC SET dataMovimentacao=CURRENT_DATE() WHERE id=$idPedido";
		$result = mysqli_query($link, $query);
	} 
	elseif (strcmp($membroexterno,$nomeAvaliador) == 0)
	{
		$query = "UPDATE $nomeTab SET unidDefMembExt=$unidadesdeferida,PontDefMembExt='$pontdeferida',observMembExt='$observacao' WHERE id=$idCriterio";
		$result = mysqli_query($link, $query);
		$query = "UPDATE TabSolicitaRSC SET dataMovimentacao=CURRENT_DATE() WHERE id=$idPedido";
		$result = mysqli_query($link, $query);
	}
	...
	{
		$query = "UPDATE $nomeTab SET unidDefMembInterno=$unidadesdeferida,PontDefMembInterno=$pontdeferida,observMembInterno='$observacao' WHERE id=$idCriterio";
		$result = mysqli_query($link, $query);
		$query = "UPDATE TabSolicitaRSC SET dataMovimentacao=CURRENT_DATE() WHERE id=$idPedido";
		$result = mysqli_query($link, $query);
	}
...
?>
			...
			<div class="row">
				<div align="center">
					<form action="CE_avaliarRSC.php?idCE=<?php echo fnEncodeID($idCE);?>" method="post">
						...
						<input type="submit" class="btn" value="Avalia próximo critério" style="border-radius: 8px; border: 3px solid #1A5321;; color: white; padding: 10px 10px; font-size: 16px; background-color: #1A5321;">
					</form>
				</div>
			</div>
			<BR>
	...
	<div style="background-color: #1A5321;">
		<BR>
		<div class="container" align="center" style="background-color: #E4EBE2;">
			<BR>
			<h3><b>Confira o Quadro de Pontuação dos avaliadores da Comissão Especial</b></h3>
			<div class="row">
				<form action="CE_quadropontuacao.php?idCE=<?php echo fnEncodeID($idCE);?>" method="post">
					...
					<input type="submit" class="btn" value="Quadro de pontuação" style="border-radius: 8px; border: 3px solid #1A5321;; color: white; padding: 10px 10px; font-size: 16px; background-color: #1A5321;">
				</form>
			</div>
			<BR>
		</div>
