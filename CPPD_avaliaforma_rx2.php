...
<?php
	...
	if ($numlinhas > 0) 
	{
		...
		$avaliador = preg_replace('/\s+/', '', $nomeAvaliadorCPPD);
//		echo nl2br("Avaliador: " . $avaliador . "\n");
	...
	$query = "SELECT avaliadorCPPD FROM TabSolicitaRSC WHERE siape='$siape'";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_assoc($result);
...
?>	
	...
	<div  style="background-color: #1A5321;">
		...
		<div class="container" style="background-color: #E4EBE2;">
			...
			<form action="CPPD_avaliaforma_rx2.php?idCPPD=<?php echo fnEncodeID($idCPPD);?>" method="post">
<?php
			$observacao = "Critério deferido na análise de forma";
?>
			<div class="row">
				<div class="coluna" align="center">
					...
					<input type="submit" class="btn" value="Deferir critério" style="border-radius: 8px; border: 3px solid #1A5321;; color: white; padding: 10px 14px; font-size: 16px; background-color: #1A5321;">
				</div>
			</div>
			</form>
			...
			<div class="row">
				<div class="coluna" align="center">
					...
					<input type="hidden" name="fatorpontuacao" value="<?php echo $fatorpontuacao;?>"/>
					<input type="hidden" name="idPedido" value="<?php echo $idPedido;?>"/>
					<input type="submit" class="btn" value="Indeferir critério" style="border-radius: 8px; border: 3px solid #D10610;; color: white; padding: 10px 14px; font-size: 16px; background-color: #D10610;">
				</div>
			</div>
			</form>
			<BR>
